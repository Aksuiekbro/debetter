// Direct MongoDB update script for Qamqor Cup tournament
...
// Get MongoDB connection from the existing server
const importServer = async () => {
  try {
    // Import the app that already has the MongoDB connection
...
const setupQamqorCup = async () => {
  try {
    // Import server to use its MongoDB connection
    await importServer();
    
    // Use the Tournament model that's already loaded by the server
    const Tournament = mongoose.model('Tournament');
    console.log('Tournament model loaded');
    ...
    // Exit explicitly since we're not part of the main server process
    process.exit(0);
  } catch (error) {
