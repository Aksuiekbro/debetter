...
const isValidCreateClusterInput = (args) => typeof args === 'object' &&
    args !== null &&
...
class AtlasProjectManager {
    ...
    apiKey;
    privateKey;
    async makeAtlasRequest(url, method, body) {
    ...
    async createAtlasCluster(input) {
        if (input.tier === 'M0') {
            return {
                content: [{
                        type: 'text',
    ...
    async setupAtlasNetworkAccess(input) {
        try {
    ...
    async getAtlasConnectionStrings(input) {
        try {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
            ...
            try {
                switch (request.params.name) {
                    case 'create_atlas_cluster':
                        result = await this.createAtlasCluster(input);
                        break;
                    case 'setup_atlas_network_access':
                        result = await this.setupAtlasNetworkAccess(input);
                        break;
                    case 'create_atlas_user':
                        result = await this.createAtlasUser(input);
                        break;
                    case 'get_atlas_connection_strings':
                        result = await this.getAtlasConnectionStrings(input);
                        break;
                    case 'list_atlas_projects':
                        result = await this.listAtlasProjects();
                        break;
                    case 'list_atlas_clusters':
                        result = await this.listAtlasClusters(input);
                        break;
                    default:
                        throw new McpError(ErrorCode.MethodNotFound, `Unknown tool: ${request.params.name}`);
                }
